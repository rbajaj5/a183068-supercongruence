...
| Result | Relationship | Status |
| --- | --- | --- |
| [Dyadic Dehn-twist sampler](related-results/DyadicDehnTwistSampler.md) | Applies exact lift coordinates to the affine shadow of a pro-$2$ Dehn-twist tower; gives an optimal uniform sampler, complete refresh spectrum, and cutoff theorem while separating this finite quotient from mapping-class-group open problems | Complete elementary corollary with exact checks; classical probability machinery and no novelty claim |
| [Fixed-generator dyadic Dehn-twist Cayley walk](related-results/DyadicDehnTwistCayleyWalk.md) | Replaces coordinate access by the fixed twists $\mathcal T_{\pm1}$ and unit scalings $5^{\pm1},-1$; cyclotomic characters give explicit two-sided spectral-gap bounds and $\Theta(4^m)$ relaxation | Complete elementary deduction with finite spectral checks; priority not claimed |
| [Affine grammar and dyadic-twist almost periodicity](related-results/DyadicTwistGrammarAlmostPeriodicity.md) | Gives affine words a unique translation-first normal form, specializes finite-group almost periodicity to dense twist averages, and computes every exact period from the gcd of the cyclotomic Fourier support | Exact grammar and Fourier deductions plus a cited Wigderson/Croot--Sisask corollary; no novelty claim |
| [Dyadic affine splitting proxy](related-results/GQ2AffineSplittingProxy.md) | Computes $H^2_{\mathrm{cont}}(\mathbb Z_2^\times,\mathbb Z_2(1))\cong\mathbb Z/2$, constructs both extension classes, and proves that splitting is exactly the parity of a sign--$5$ cross-commutator; also distinguishes Yablo incompatibility from profinite/discrete failure | Complete standard cohomology calculation and explicit model with 2.35 million exact checks; the actual $\operatorname{Out}(D_0)$ kernel pushout remains open |
