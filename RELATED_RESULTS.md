...
| [Cooper level-11 rare primes](related-results/CooperLevel11RarePrimes.md) | Isolates the $n=1$ obstruction and an 8,300-case first-order law that would make it equivalent to every $r=1$ congruence; finds a three-branch base-$3$ recursion modulo $9$ in 30,003 exact checks; rejects the naive $X_0(11)$ trace-$2$ explanation; identifies a conditional finite-automaton proof route once a rational diagonal is exhibited | Reproducible computational theorem targets; general proofs and the automaton premise are not yet established |
