...
\[
\left|\operatorname{Sur}(G_{\mathbb Q_2},G)\right|
\]
...
\[
\left|\operatorname{Sur}(G_{\mathbb Q_2},C_{2^m})\right|
=
\begin{cases}
7,&m=1,\\
3\cdot2^{2m-1},&m\ge2.
...
After division by
\(\left|\operatorname{Aut}(C_{2^m})\right|\), the number of cyclic
...
The first genuinely nonabelian targets are the next useful boundary. The
presentation suggests counting them by:
...
Dihedral, quaternion, and extraspecial \(2\)-groups are likely the smallest
targets on which the quadratic layer becomes visible. That would connect
this elementary formula to the Fourier and Gauss-sign machinery in the main
paper.
