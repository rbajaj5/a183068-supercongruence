...
| 1D | [Gaussian base change for Jacobian collisions](GaussianBaseChangeJacobianCollisions.md) | Moves the collision theorem to prime ideals; split Gaussian primes use \(F\), inert primes use \(F^2\), and the degree-four Artin obstruction disappears in the inert tower | Complete theorem with exact split/inert checks; priority preliminary |
| 1E | [Finite-field counts for the Fable Jacobian counterexample](JacobianCounterexampleFiniteFieldCounts.md) | Uses the same adjacent-extension viewpoint to organize cubic fibers, collisions, and Frobenius counts of the new counterexample | Complete elementary theorem with exact checks; priority preliminary |
| 1F | [Degree-four Jacobian Frobenius obstruction](JacobianDegreeFourFrobeniusObstruction.md) | Shows how quadratic Artin factors obstruct raw adjacency and how two-step or corrected towers recover exact congruences | Complete elementary theorem with exact checks; priority preliminary |
| 1G | [Degree-five elliptic Frobenius packet](JacobianDegreeFiveEllipticFrobenius.md) | Shows the next transition: a tangent cubic contributes a non-CM elliptic trace, and removing the full Frobenius packet leaves an exact \(2r-2\) tower | Complete theorem with direct finite-field and tower checks; priority preliminary |
| 1H | [Degree-six genus-three Frobenius obstruction](JacobianDegreeSixGenusThree.md) | Gives the first explicit higher-genus packet and a permanent raw obstruction at \(p=13\) | Complete theorem with extension-field and symbolic checks; priority preliminary |
| 1I | [Degree-seven genus-six Frobenius automaton](JacobianDegreeSevenGenusSix.md) | Produces the period-\(156\) example that motivates the general fixed-precision automaton | Complete theorem with exact counts through \(\mathbf F_{5^6}\); priority preliminary |
...
- the fixed-precision Frobenius automaton theorem, which makes every good
  ...
  density;
- the prime-ideal base-change theorem and its split/inert Gaussian
  specialization for the degree-four collision tower;
...
| Gaussian base change for Jacobian collisions | `verification/related/verify_gaussian_base_change_collisions.py` |
