...
| 1B-AUG5 | [First-defect kernel for negative-binomial prefixes](BalaAugustFirstDefectKernel.md) | Extracts the normalized $p^{3r}$ defect as one explicit Cartier moment, classifies sharpness by its residue, and reduces scale stabilization to one Frobenius-fixed kernel identity; a canonical primitive makes coefficientwise Frobenius fixedness the stronger target | Complete first-layer reduction for $p\ge5$; 2,858 exact checks; the lifted square and cube-transgression lemmas remain open |
