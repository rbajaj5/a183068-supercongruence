...
| 1B | [Peter Bala's OEIS queue](BalaOeisSupercongruenceQueue.md) | Proves the A365029 boundary family and first two adjacent levels, all three A375178--A375180 prime-level odd-power families, and the A112028/A219562 fifth-power boundaries; closes A333593 via Coster and Jacobsthal--Kazandzidis | Four complete elementary proof packets, published-theorem reductions, and exact checks; priority preliminary |
| 1B-E3 | [Enhanced A112028 cubic tower](A112028EnhancedTower.md) | Proves the full $p^{3r+3}$ tower through a two-digit unit shell and two finite-field shell cancellations | Complete elementary proof candidate for $p\ge7$; exact checks pass |
| 1B-E4 | [Enhanced A219562 quartic tower](A219562EnhancedTower.md) | Proves the full $p^{3r+3}$ tower through one reciprocal-square critical shell | Complete elementary proof candidate for $p\ge5$; exact checks pass |
| 1B-O | [Complete A375178 odd-power tower](A375178EnhancedOddPowerTower.md) | Proves the full $p^{3r+2m+1}$ tower for every odd exponent $2m+1$ by a universal final-shell harmonic involution | Complete elementary proof candidate; 6,211 exact checks pass |
| 1B-SO | [Complete signed odd-power towers](A375179A375180SignedOddPowerTowers.md) | Proves the full $p^{3r+2m+1}$ towers on A375179 and A375180, uniformly for every prime-to-$p$ dilation, by a negative-slope generalized-binomial reduction | Complete elementary proof candidate; 17,896 exact checks pass |
| 1B-A365D | [A365029 all-level digit reduction](A365029AllLevelDigitReduction.md) | Gives an exact recursive factorization of every unit-index summand and reduces the full tower to one complete-digit cancellation, with separate lower/upper half vanishing | Exact reduction; verified through four levels; final digit lemma open |
| 1B-A219 | [Enhanced A219562 tower](A219562EnhancedTower.md) | Splits the quartic shifted-binomial difference into valuation strata and reduces the sole critical shell to the inverse-square sum on \(\mathbb F_p^\times\) | Complete \(p^{3r+3}\) proof candidate for every \(p\ge5\), \(r\ge2\); 4,231 exact checks |
...
| 1B-I3 | [A364183 affine-Landau integrality and tower](A364183AffineLandauTower.md) | Resolves A295479 at half-index by a two-case, twelve-row affine carry lemma and factors it into eleven half-binomial towers | Complete elementary proof candidate; 2,002,942 exact checks; priority pending |
| 1B-I4 | [Uniform Bober half-index theorem](BoberHalfIndexIntegralityTheorem.md) | Translates the ordinary Landau function by $1/2$ at odd primes and controls the binary digit sum at two | Closes all eleven half-index variants; 883,740 exact checks; priority pending |
| 1B-I5 | [Bober denominator-boundary theorem](BoberDenominatorBoundaryTheorem.md) | Translates the ordinary Landau function for denominators three/four and controls the exceptional primes by digit sums | Closes the last four source-visible fractional variants; 565,742 exact checks; priority pending |
...
| 1B-AUG5 | [First-defect kernel for negative-binomial prefixes](BalaAugustFirstDefectKernel.md) | Extracts the normalized $p^{3r}$ defect as one Cartier moment and proves its coefficientwise Frobenius fixedness through a finite logarithm and piecewise-linear Green kernel | Complete first-residue stabilization theorem for $p\ge5$; 3,902 exact checks; the growing refinement is closed in 1B-AUG6 |
| 1B-AUG6 | [Higher August normalized-defect lift](BalaAugustHigherDefectLift.md) | Proves the sharp growing modulus by an exact three-level reduction, a quartic coefficient lift, and a second Cartier-connection identity whose logarithmic correction cancels the cubic Hessian term | Complete proof candidate for $p\ge5$; 2,200 exact checks; independent review pending |
...
| 1B-B52 | [Bober sporadic factorial-ratio packet](BoberSporadicFactorialRatioPacket.md) | Applies the balanced, translated-Landau, digit-sum, and rational-gamma engines to all 52 Bober ratios and Bala's 15 visible fractional-index variants | Ordinary 52-record and all 15 source-visible fractional towers proved for $p\ge5$; A295464 awaits source text |
...
| 1B-S | [110-record proof-campaign ledger](Bala110ProofCampaign.md) | Gives every census record a route, conservative status, evidence pointer, and next proof operation | Complete machine-readable ledger; 48 proved here, 20 source-closed, 35 partial, 7 with no explicit open target, no wholly untreated records, and none queued |
...
| 1B-A376 | [A376 Apéry-companion reduction](A376AperyCompanionReduction.md) | Closes all stated A376458 moduli, refutes the A376466 shifted tower, and supplies their common coefficient framework | Complete A376458 resolution plus exact A376466 counterexample and bridge |
| 1B-A376B | [A376466 ordinary cubic tower](A376466OrdinaryTower.md) | Cancels the unit harmonic defect and closes the scaled boundary with an exact Legendre-row recurrence | Complete proof for every `p>=5`; 16,303 exact checks |
...
| 4D-G | [General cyclotomic rational-framing tower](CyclotomicRationalFramingTower.md) | Proves the all-but-finitely-many-primes principle on A228960 and A350383 for every cyclotomic quotient, coefficient slope, and integral power parameter | Complete elementary proof candidate; 2,095 exact checks; priority unestablished |
...
| 4E-M2 | [Cubic Euler-product Cartier square](EulerProductCubicCartierMoments.md) | Reduces A023871, A023873, A206622, A283271, and A380290 at every level to one formal-derivative/Cartier-square obstruction; both weighted moments and all logarithmic degrees at least three are discharged | Exact all-level reduction; 5,419 checks; the one formal-derivative estimate remains open |
...
| A364183 affine-Landau theorem | `verification/related/verify_a364183_affine_landau.py` |
| Uniform Bober half-index theorem | `verification/related/verify_bober_half_index_integrality.py` |
...
| Bala August higher normalized-defect theorem | `verification/related/verify_bala_august_higher_defect.py` |
...
| General cyclotomic rational framing | `verification/related/verify_cyclotomic_rational_framing.py` |
