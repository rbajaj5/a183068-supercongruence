...
| 1B-S | [110-record proof-campaign ledger](Bala110ProofCampaign.md) | Gives every census record a route, conservative status, evidence pointer, and next proof operation | Complete machine-readable ledger; 17 proved here, 12 source-closed, 15 partial, 3 with no explicit open target, and 63 queued |
...
| 4E-M | [Modular-product prime-coefficient packet](ModularProductPrimeCoefficientPacket.md) | Proves all displayed conjectures on seven route-M records and supplies four quadratic baselines by combining one universal first-coefficient formula with the colored Euler-product theorem | Complete elementary packet; 256 exact checks; three route-M records remain without a proved baseline |
