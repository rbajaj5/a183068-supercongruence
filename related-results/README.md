...
| 1B-AUG5 | [First-defect kernel for negative-binomial prefixes](BalaAugustFirstDefectKernel.md) | Extracts the normalized $p^{3r}$ defect as one explicit Cartier moment, classifies sharpness by its residue, and reduces scale stabilization to one Frobenius-fixed kernel identity | Complete first-layer reduction for $p\ge5$; 1,474 exact checks; the final kernel-moment identity remains open |
