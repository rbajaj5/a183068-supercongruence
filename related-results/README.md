...
| 1B-AUG5 | [First-defect kernel for negative-binomial prefixes](BalaAugustFirstDefectKernel.md) | Extracts the normalized $p^{3r}$ defect as one explicit Cartier moment, classifies sharpness by its residue, and reduces scale stabilization to one Frobenius-fixed kernel identity; a second unit-block lift proves the quadratic primitive is Cartier-fixed | Complete first-layer reduction and lifted-square theorem for $p\ge5$; 2,858 exact checks; only the reciprocal-cube transgression remains open |
