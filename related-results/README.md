...
| 1B-P | [Catalan-ballot prime-level theorem](CatalanBallotPrimeLevelTheorem.md) | Uses complete-block alternating harmonic cancellation to prove the \(p^3\) prime slice for every odd ballot power | Complete elementary proof candidate; 557 exact checks; higher \(n,r\) towers remain open |
...
| 1B-S | [110-record proof-campaign ledger](Bala110ProofCampaign.md) | Gives every census record a route, conservative status, evidence pointer, and next proof operation | Complete machine-readable ledger; 29 proved here, 12 source-closed, 22 partial, 3 with no explicit open target, and 44 queued |
