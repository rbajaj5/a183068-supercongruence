...
## 5. Atomic spectra and the stabilized Fermat defect

The Gelfand transform makes the preceding finite-dimensional spectral
description arithmetic. Let

```math
\nu=\sum_{j=1}^s w_j\delta_{\lambda_j},
\qquad
w_j\in\mathbb Z_{(p)},\quad \lambda_j\in\mathbb Z,
\tag{19}
```

be a finite $p$-integral atomic measure and put

```math
a_N=\int z^N\,d\nu(z)=\sum_jw_j\lambda_j^N.
\tag{20}
```

The weights may be signed. Here $\mathbb Z_{(p)}$ denotes the rationals with
denominator prime to $p$.
For a $p$-adic unit $u$, write

```math
q_p(u)=\frac{u^{p-1}-1}{p}
```

for its Fermat quotient.

### Theorem 3 (scale-independent spectral defect)

Let $p$ be odd. For every $n\ge1$ and $r\ge2$,

```math
D_{p,r}(n)=\frac{a_{np^r}-a_{np^{r-1}}}{p^r}
\in\mathbb Z_{(p)}
```
...
D_{p,r}(n)\equiv
\sum_{p\nmid\lambda_j}
w_j\lambda_j^nq_p(\lambda_j^n)
\pmod p.
}
\tag{21}
```

In particular, the residue is independent of $r$. If it is nonzero, then

```math
v_p(a_{np^r}-a_{np^{r-1}})=r
\qquad(r\ge2).
\tag{22}
```
...
Put $u=\lambda_j^n$. If $p\nmid u$, then
$u^{p-1}=1+pq_p(u)$ and

```math
u^{p^r}-u^{p^{r-1}}
=u^{p^{r-1}}
\left((1+pq_p(u))^{p^{r-1}}-1\right).
```

For odd $p$, the binomial theorem gives

```math
\frac{(1+pq)^{p^{r-1}}-1}{p^r}\equiv q\pmod p,
```

while $u^{p^{r-1}}\equiv u\pmod p$. This proves the unit contribution in
(21). If $u=0$, its contribution is zero. If $p\mid u$ and $u\ne0$, then

```math
v_p(u^{p^r}-u^{p^{r-1}})=p^{r-1}v_p(u)\ge r+1
```

for $r\ge2$, so the nonunit contribution vanishes after division by $p^r$
and reduction modulo $p$. Summing with the $p$-integral weights proves the
theorem. $\square$

For a finite group with an integer-valued character and $p\nmid|K|$, the
spectral measure is

```math
\nu_V=\sum_C\frac{|C|}{|K|}\,\delta_{\chi_V(C)},
\tag{23}
```

where $C$ runs over conjugacy classes. Thus (21) is a finite obstruction
test for every proposed enhancement of (13). It is strictly stronger than
knowing the spectral radius: it uses the full atomic spectrum and its Haar
weights.

For example, take $K=S_3$ and $V=\mathbf1\oplus\mathrm{Std}$. The character
values on the three conjugacy classes are $3,1,0$, with weights
$1/6,3/6,2/6$. For every prime $p\ge5$,

```math
D_{p,r}(n)\equiv
\frac{3^n}{6}q_p(3^n)\pmod p
\qquad(r\ge2).
\tag{24}
```

At $p=5,n=1$ this residue is $3$, so the exponent is exactly $r$ at every
level $r\ge2$. More generally,
$q_p(3^n)\equiv n q_p(3)\pmod p$ isolates the familiar Wieferich-type
exceptional primes and the trivial exceptional indices $p\mid n$.

## 6. The nonabelian boundary is real
...
$S_3$, the scale-independent atomic defect formula, and the $SU(2)$ binary
counterexample. These checks are regression tests; Sections 1--7 contain the
proof.

## 9. Sources and priority boundary

- N. Bourbaki, *Théories spectrales*, Chapters I--II, 2nd ed., Springer,
  2019: spectra and the Gelfand transform in Chapter I, Section 1; spectral
  radius in Chapter I, Section 2, no. 3; normalized Haar measure and Fourier
  characters in Chapter II, Section 1.
